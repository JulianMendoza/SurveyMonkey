...
public class AdminControllerTest {
	...
	public void getNonExistantSurveyPageTest() throws Exception {
		this.mockMvc.perform(get("/survey/non-existant-survey-code")).andDo(print()).andExpect(status().isOk()).andExpect(content().string(containsString("Sorry, there is no survey with that link.")));
	}
	...
	public void getExistingSurveyPageTest() throws Exception {
		...
		this.mockMvc.perform(get("/survey/TestCode")).andDo(print()).andExpect(status().isOk()).andExpect(content().string(containsString(survey.getTitle())));
	...
	public void surveyCreationTitleTest() throws Exception {
		this.mockMvc.perform(get("/surveyQuestions?title=coolbeans")).andDo(print()).andExpect(status().isOk()).andExpect(content().string(containsString("coolbeans")));
	}
	...
	public void surveyResultPageTest() throws Exception {
		...
		this.mockMvc.perform(post(url)).andDo(print()).andExpect(status().isOk()).andExpect(content().string(containsString(survey.getSurveyCode())));
		surveyRepository.delete(survey);
	}
	...
	public void surveyResultJSONTest() throws Exception {
		...
		this.mockMvc.perform(post("/surveyResult").contentType(MediaType.APPLICATION_JSON).content(str)).andExpect(content().string(containsString("What is life?")));
		surveyRepository.delete(survey);
	}
	...
	public void surveyResultsWithoutPasswordTest() throws Exception {
		...
		dr.setData(survey.getSurveyCode());
		String str = asJsonString(dr);
		this.mockMvc.perform(post("/surveyResultsWithoutPassword").contentType(MediaType.APPLICATION_JSON).content(str)).andExpect(content().string(containsString("This is a question?")));
		surveyRepository.delete(survey);
	}
